...
import { ReadingStatusEmoji } from '@components/_ui/EmojiSelector';
...
const BookCardMdx = ({ data }: MdxFrontmatter) => {
  const image = getImage(data.cover);
  const bookUrl = `/${data.slug}`;
  const readingStatusEmoji = ReadingStatusEmoji(String(data.readingStatus));
